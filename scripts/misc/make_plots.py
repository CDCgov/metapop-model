...
replicate_0_results = results.filter(
    (pl.col("replicate") == 0)
    & (pl.col("initial_coverage_scenario") == "optimistic")
    & (pl.col("k_21") == 0.01)
)
...
g.map_dataframe(
    sns.histplot, x="Y", hue="group", multiple="stack", bins=bins, palette=plot_cols
)
...
grouped_results = (
    filtered_results_df.groupby(["replicate", "initial_coverage_scenario", "k_21"])
    .agg({"Y": "sum"})
    .reset_index()
)
