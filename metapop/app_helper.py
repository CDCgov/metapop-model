...
def get_helpers(parms=None):
    ...
    defaults = dict(
        ...
        calculator_on="If turned on, the simulator will use age distribution data and age-specific immunity values to calculate an overall population immunity value and use this number for outbreak simulation. Default age distribution values are 2023 ACS 5-year national estimates from the US Census Bureau and default immunity data are national MMR vaccination coverage estimates from CDC (see our Behind the Model for more details and data sources). Note that even though there are age-specific values in the calculator, this toggle will not add age structure to the underlying transmission model.",
...
def calc_immunity():
    ...
    if st.session_state.table_changed:
        ...
        if has_missing_coverage:
            ...
            message = f"Based on these values, the estimated baseline immunity in this community is {immunity_text}%"
        st.success(message)
