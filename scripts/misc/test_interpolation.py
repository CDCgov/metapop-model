...
baseline_immun = get_baseline_immunity(
                df_pop["percentage"], df_coverage["coverage"]
            )
step_baseline_immune = get_step_immunity(df_pop["percentage"],
                                         df_coverage["coverage"])



x = pd.concat([pd.DataFrame([df_pop["percentage"][0]]), df_pop["percentage"]], ignore_index=True)
...
y = pd.concat([pd.DataFrame([0]), df_coverage["coverage"], pd.DataFrame([df_coverage.loc[3, "coverage"]])], ignore_index=True)
...
for y in range(0, 100, 10):  
    plt.plot(range(0, 31), [y] * len(range(0,31)), "--", lw=0.5, color="black", alpha=0.25)  
