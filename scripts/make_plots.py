...
plot_cols = ["blue", "green", "orange"]
sns.set_context("notebook", font_scale=1.5)

#### Visualization of one simulation ####
# Filter the data for replicate == 0 and initial_coverage_scenario == "low"
replicate_0_results = results.filter((pl.col("replicate") == 0) & (pl.col("initial_coverage_scenario") == "low") & (pl.col("connectivity_scenario") == 1.0))
...
bins = np.linspace(min_value, max_value, 41)  # 40 bins

# Create the faceted histogram with the same bin width for all histograms
g = sns.FacetGrid(filtered_results_df, row="initial_coverage_scenario", col = "connectivity_scenario", sharex=True, sharey=True, height=4, aspect=3)
...
g.savefig('output/final_size.png')
