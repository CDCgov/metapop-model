...
def get_helpers(parms=None):
    ...
    defaults = dict(
        ...
        calculator_on="If turned on, the simulator will use age distribution data and age-specific immunity values to calculate an overall population immunity value and use this number for outbreak simulation. Default age distribution values are 2020 national estimates from the US Census Bureau and default immunity data are national MMR vaccination coverage estimates from CDC (see our Behind the Model for more details and data sources). Note that even though there are age-specific values in the calculator, this toggle will not add age structure to the underlying transmission model.",
